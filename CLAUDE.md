...
- **Parallelize code changes across agents**: After planning, split implementation into independent change sets grouped by file. Launch multiple general-purpose Task agents in a single message — each agent owns specific file(s) and receives self-contained instructions with full context. No two agents should edit the same file. After all agents complete, run `npm run build` to verify integration. Typical groupings: (A) types.ts, (B) handler files, (C) battleStore.ts, (D) buffConditions.ts.
